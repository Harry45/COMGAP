...
class forwardModel(moped):
    def __init__(
        ...
        assert method in [
            ...
            "first",
            "second",
        ], 'The method must be either "exact" or "first" or "second".'
        ...
        self.expansion_point = torch.from_numpy(
            hp.load_arrays("matrices", "expansion_point")
        )
        self.moped_vectors = torch.from_numpy(
            hp.load_arrays("matrices", "moped_vectors")
        )

        if self.emulator:
            # load the GPs
            self.gps = hp.read_list(
                st.GP_PATH, "gps_" + self.method + "_modules_" + str(nlhs)
            )
    ...
    def compression(self, parameters: torch.tensor) -> torch.tensor:
        ...
        if self.emulator and self.variance:
            ...
            for i in range(self.ndim):
                mean[i], var[i] = self.gps[i].scaled_prediction(parameters, variance=True)
        ...
        if self.emulator and not self.variance:
            ...
            mean = [
                self.gps[i].prediction(parameters, variance=False).item()
                for i in range(self.ndim)
            ]
