...
class ScaleDist:
    ...
    def __init__(self, nlhs, factor):
        ...
        if os.path.isfile(fname):
            ...
            raise Exception('LHS samples not found. Run the LHS script first and store the output in the folder lhs/.')
    ...
    def distributions(self, factor: float = 5.0) -> dict:
        ...
        p_nuisance = td.MultivariateNormal(self.mle[2:], covariance_matrix=cov_nui, validate_args=False)
    ...
    def scaling(self):
        ...
        samples_nuisance = self.mle[2:].view(st.n_nui, 1) + cho_nui @ self.lhs[:, 2:].t()
