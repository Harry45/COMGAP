...
class AppMag(object):
    ...
    def load_data(self):
        ...
        self.cov_eta = torch.tensor(sum([fits.open(mat)[0].data for mat in glob.glob(st.data_path + 'C*.fits')]))
    ...
    def load_expansion(self):
        ...
        if os.path.isfile(fname):
            ...
            raise Exception('The maximum likelihood estimator is not available. Please run the optimisation procedure.')
    ...
    def theory_covariance(self, parameters: torch.tensor) -> torch.tensor:
        ...
        sigma_pecvel = (5 * 150 / 3E5) / (np.log(10.) * self.sigma_mu[:, 2])
        cov += torch.eye(self.ndata) * (self.sigma_mu[:, 0] ** 2 + self.sigma_mu[:, 1] ** 2 + sigma_pecvel ** 2)
    ...
    def theory_only(self, parameters: torch.tensor) -> torch.tensor:
        ...
        for i in range(self.ndata):
            ...
            light_params = torch.tensor([self.log_stellar_mass[i], self.x1[i], self.color[i]])

            predicted[i] = cf.forward(parameters, redshift, light_params)

        return predicted

    def theory(self, parameters: torch.tensor,
               hessian: bool = False) -> Union[torch.tensor, torch.tensor, torch.tensor]:
        ...
        for i in range(self.ndata):
            ...
            predicted[i], gradients[i] = cf.first_derivative(parameters, redshift, light_params)
    ...
    def theory_expansion(self, parameters: torch.tensor) -> Tuple[torch.tensor, torch.tensor, torch.tensor]:
    ...
    def approximate_theory(self, parameters: torch.tensor, second: bool = False) -> torch.tensor:
        ...
        if not hasattr(self, 'theory_mu_0'):
            raise Exception('The model, gradient and Hessian have not been computed at the expansion point.')
